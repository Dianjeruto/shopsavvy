...
import { resolveDemoProduct } from '@/lib/demo-catalog';
...
const ProductDetail: React.FC = () => {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        const { data, error } = await supabase.from('ecom_products').select('*, variants:ecom_product_variants(*)').eq('handle', handle).single();
        const fallback = resolveDemoProduct(handle || '');
        ...
        if (source.has_variants && variants.length === 0) {
          ...
          variants = vd || []; source.variants = variants;
        }
        ...
        const { data: rel } = await supabase.from('ecom_products').select('*, variants:ecom_product_variants(*)').eq('product_type', source.product_type).neq('id', source.id).eq('status', 'active').limit(4);
        setRelated(rel || []);
      } catch {
        const fallback = resolveDemoProduct(handle || '');
        if (!fallback) return;
        ...
        if (fallbackVariants.length) {
          const first = fallbackVariants.find((v: any) => v.inventory_qty == null || v.inventory_qty > 0) || fallbackVariants[0];
          setSelectedVariant(first); setSelectedSize(first?.option1 || '');
        }
        setRelated([]);
      }
