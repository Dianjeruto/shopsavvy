...
import { resolveDemoProductsByIds } from '@/lib/demo-catalog';
...
const Wishlist: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const { data, error } = await supabase.from('ecom_products').select('*, variants:ecom_product_variants(*)').in('id', wishlist);
        if (error || !data?.length) {
          setProducts(resolveDemoProductsByIds(wishlist));
        } else {
          setProducts(data);
        }
      } catch {
